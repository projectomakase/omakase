...
public class JobMessagesResource {
    ...
    public JobMessagesResource(JobManager jobManager, RepresentationConverter<MessageModel, Message> messageRepresentationConverter, MessageQuerySearchConverter querySearchConverter,
                               UriInfo uriInfo) {
    ...
    public Response getJobs(@QueryParam("page") @DefaultValue("1") int page, @QueryParam("per_page") @DefaultValue("10") int perPage,
                            @DefaultValue("false") @QueryParam("only_count") boolean onlyCount, @PathParam("jobId") String jobId) {
        ...
        SearchResult<Message> searchResult = jobManager.findJobMessages(jobId, search);
        if (!onlyCount) {
