 ...
 */
public class VariantFileSearchBuilder extends Search.Builder {
    ...
    static {
        SUPPORTED_SORT_ATTRIBUTES = ImmutableList.of(VariantFile.ID, VariantFile.NAME, VariantFile.ORIGINAL_FILENAME, VariantFile.CREATED);

        ImmutableMultimap.Builder<String, Operator> mapBuilder = ImmutableMultimap.builder();
        mapBuilder.putAll(VariantFile.NAME, Operator.EQ, Operator.LIKE);
        mapBuilder.putAll(VariantFile.ORIGINAL_FILENAME, Operator.EQ, Operator.LIKE);
        mapBuilder.putAll(VariantFile.SIZE, Operator.EQ, Operator.GT, Operator.GTE, Operator.LT, Operator.LTE);
        mapBuilder.putAll(VariantFile.CREATED, Operator.EQ, Operator.GT, Operator.GTE, Operator.LT, Operator.LTE);
        mapBuilder.putAll(VariantFile.TYPE, Operator.EQ, Operator.NE);
        SUPPORTED_CONDITIONS = mapBuilder.build();
    }

    @Override
    public Search build() {
        super.condition(new SearchCondition(VariantFile.TYPE, Operator.NE, "CHILD"));
        return super.build();
    }
