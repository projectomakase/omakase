...
/**
 * JAX-RS Subresource for /assets/{assetId}/variants
 *
...
public class VariantsResource {
    ...
    private final VariantQuerySearchConverter querySearchConverter;
    private final UriInfo uriInfo;

    public VariantsResource(ContentManager contentManager, RepresentationConverter<VariantModel, Variant> variantRepresentationConverter, VariantQuerySearchConverter querySearchConverter,
                            UriInfo uriInfo) {
    ...
    @GET
    @GZIP
    public Response getAssetVariants(@QueryParam("page") @DefaultValue("1") int page, @QueryParam("per_page") @DefaultValue("10") int perPage,
                                     @DefaultValue("false") @QueryParam("only_count") boolean onlyCount, @PathParam("assetId") String assetId) {
        ...
        SearchResult<Variant> searchResult = contentManager.findVariants(assetId, search);
        if (!onlyCount) {
            Collection<VariantModel> variantModels = variantRepresentationConverter.from(uriInfo, searchResult.getRecords());
            PaginationLinks paginationLinks = new PaginationLinks(uriInfo, page, perPage, searchResult.getTotalRecords());
            PaginatedEnvelope<VariantModel> envelope =
                    new PaginatedEnvelope<>(page, perPage, paginationLinks.getTotalPages(), searchResult.getTotalRecords(), variantModels, paginationLinks.get());
            ...
            PaginatedEnvelope<VariantModel> envelope = new PaginatedEnvelope<>(null, null, null, searchResult.getTotalRecords(), ImmutableList.of(), null);
            return Response.ok(envelope).build();
        }
    }

    @POST
    public Response createAssetVariant(@PathParam("assetId") String assetId, VariantModel variantModel) {
        Variant createdVariant = contentManager.createVariant(assetId, variantRepresentationConverter.to(uriInfo, variantModel));
        return Response.created(UriBuilder.fromUri(uriInfo.getAbsolutePath()).path(createdVariant.getId()).build(createdVariant.getId())).entity(new ResponseStatusModel(ResponseStatusValue.OK))
                .build();
    }
}
