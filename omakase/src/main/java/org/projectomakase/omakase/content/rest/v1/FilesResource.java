...
/**
 * JAX-RS Subresource for /assets/{id}/variants/{id}/files
 *
...
public class FilesResource {
    ...
    private final FileQuerySearchConverter querySearchConverter;
    private final UriInfo uriInfo;

    public FilesResource(ContentManager contentManager, RepresentationConverter<FileModel, VariantFile> representationConverter, FileQuerySearchConverter querySearchConverter, UriInfo uriInfo) {
    ...
    @GET
    @GZIP
    public Response getFiles(@QueryParam("page") @DefaultValue("1") int page, @QueryParam("per_page") @DefaultValue("10") int perPage,
        ...
        SearchResult<VariantFile> searchResult = contentManager.findVariantFiles(assetId, variantId, search);
        if (!onlyCount) {
            Collection<FileModel> fileModels = representationConverter.from(uriInfo, searchResult.getRecords());
            PaginationLinks paginationLinks = new PaginationLinks(uriInfo, page, perPage, searchResult.getTotalRecords());
            PaginatedEnvelope<FileModel> envelope =
                    new PaginatedEnvelope<>(page, perPage, paginationLinks.getTotalPages(), searchResult.getTotalRecords(), fileModels, paginationLinks.get());
            ...
            PaginatedEnvelope<FileModel> envelope = new PaginatedEnvelope<>(null, null, null, searchResult.getTotalRecords(), ImmutableList.of(), null);
